...
#
# You'll only have to do this once, however.
#
...
echo "-------------------------------------------------------------------------"
echo "Harvesting zipfile URLs for format [${FILE_FORMAT}] in language [${LANGUAGE}]."
echo "-------------------------------------------------------------------------"
...
if [ ! -f "${ZIP_LIST}" ] ; then
  ...
  wget -w 2 -m "http://www.gutenberg.org/robot/harvest?filetypes[]=${FILE_FORMAT}&langs[]=${LANGUAGE}"

  # Process the downloaded HTML link lists into a single sorted file of zipfile
  # URLs, one per line.
  grep -oh 'http://[a-zA-Z0-9./]*.zip' "${DIR}/www.gutenberg.org/robot/harvest*" | sort | uniq > "${ZIP_LIST}"
...
do
  ...
  if [ ! -f "${ZIP_FILE}" ] ; then
    ...
    # Play nice with a delay and thus not get banned.
    sleep 2
  else
